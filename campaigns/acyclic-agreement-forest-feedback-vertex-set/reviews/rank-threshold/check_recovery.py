...
def check(left, right, labels):
    ...
    for clique in sorted(cliques, key=lambda c: (-len(c), sorted(c))):
        if occupied.intersection(clique):
            continue
        occupied.update(clique)
        bound += len(clique)-1
        extra = model.new_bool_var('extra'+str(len(surplus)))
        model.add(sum(deleted[v] for v in clique) == len(clique)-1+extra)
        surplus.append(extra)
    surplus.extend(deleted[v] for v in vertices if v not in occupied)
    solver = cp_model.CpSolver()
    solver.parameters.num_search_workers = 1
    for additional in range(len(vertices)-bound+1):
        trial = model.clone()
        trial.add(sum(surplus) == additional)
        status = solver.solve(trial)
        if status == cp_model.OPTIMAL:
            model = trial
            break
        assert status == cp_model.INFEASIBLE
    else:
        raise AssertionError('deleting every vertex must be feasible')
    target_optimum = sum(solver.value(v) for v in deleted.values())
    assert target_optimum == bound + additional
